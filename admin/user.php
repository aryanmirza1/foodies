                      ...
                      <tbody>
                        <?php $sr=1; while($row=mysqli_fetch_assoc($result)): ?>
                          <?php
                            ...
                            $postal    = htmlspecialchars($row["postal_code"] ?? "");
                            $avatar    = trim((string)($row["avatar_url"] ?? ""));
                            $passhash  = htmlspecialchars($row["password_hash"] ?? "");
                            $status    = (int)($row["status"] ?? 0);
                            $createdAt = !empty($row["created_at"]) ? date("M d, Y H:i", strtotime($row["created_at"])) : "";

                            // avatar src: accept absolute (http/https or /rooted), else treat as relative to web root
                            if ($avatar === '') {
                            ...
                            $address = htmlspecialchars($addr1 . ($addr2 ? " • ".$addr2 : ""));
                          ?>
                          <tr>
                            <td><?php echo $sr++; ?></td>
                            ...
                            <td class="text-muted small"><?php echo $address; ?></td>
                            <td class="text-muted small"><?php echo $city; ?><?php echo $city && ($state||$postal) ? ' / ' : ''; ?><?php echo $state; ?><?php echo ($state||$city) && $postal ? ' / ' : ''; ?><?php echo $postal; ?></td>
                            <td class="text-muted small"><?php echo $createdAt; ?></td>

                            <!-- Masked password with tooltip (full hash on hover) -->
                            <td>
                              <span class="text-monospace" data-bs-toggle="tooltip" title="<?php echo $passhash; ?>">
                                ••••••••
                              </span>
                            </td>
